...
public class KMeans {
	 ...
	 */
	public KMeans(int numClusters, int numFeatures, List<edu.carleton.comp4106.model.User> userList) {
		changed = true;
		...
		for(int i = 0; i < userList.size(); i++) {
			edu.carleton.comp4106.model.User userToTransform = 
					userList.get(i);
			
			Integer id = userToTransform.getId();
			
			users[i] = new User(String.valueOf(id), no_features, numClusters);
			
			users[i].features[0] = userToTransform.getEDM();
			users[i].features[1] = userToTransform.getHipHop();
			users[i].features[2] = userToTransform.getRNB();
			users[i].features[3] = userToTransform.getPop();
			users[i].features[4] = userToTransform.getRock();
			users[i].features[5] = userToTransform.getReggae();
			users[i].features[6] = userToTransform.getMetal();
			users[i].features[7] = userToTransform.getCountry();
		}
	...
	public List<edu.carleton.comp4106.model.User> execute() {
		System.out.println();
	...
	public List<edu.carleton.comp4106.model.User> convertUsers(List<User>[] clusters) {
		List<edu.carleton.comp4106.model.User> allClusteredUsers = 
				new ArrayList<edu.carleton.comp4106.model.User>();
		
		for(int i = 0; i < clusters.length; i++) {
			List<User> cluster = clusters[i];
			
			List<edu.carleton.comp4106.model.User> clusteredUsers = 
	...
	public List<edu.carleton.comp4106.model.User> userAdapter(List<User> kMeansUsers, int clusterNumber) {
		List<edu.carleton.comp4106.model.User> clusterUsers = 
				new ArrayList<edu.carleton.comp4106.model.User>();
		
		for(User kMeansUser: kMeansUsers) {
			edu.carleton.comp4106.model.User user = 
					new edu.carleton.comp4106.model.User();
			
			// Transfer Information
			user.setId(Integer.valueOf(kMeansUser.name));
			user.setCluster(clusterNumber);
			user.setEDM(kMeansUser.features[0]);
			user.setHipHop(kMeansUser.features[1]);
			user.setRNB(kMeansUser.features[2]);
			user.setPop(kMeansUser.features[3]);
			user.setRock(kMeansUser.features[4]);
			user.setReggae(kMeansUser.features[5]);
			user.setMetal(kMeansUser.features[6]);
			user.setCountry(kMeansUser.features[7]);
